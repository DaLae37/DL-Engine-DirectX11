...
private :
	...
	std::unordered_map<const wchar_t*, WRL::ComPtr<ID2D1Bitmap>> d2dTextureMap;
	std::unordered_map<const wchar_t*, WRL::ComPtr<ID3D11Texture2D>> d3dTextureMap;
...
public :
	...
	WRL::ComPtr<ID2D1Bitmap> LoadD2DTextureFromFile(const wchar_t* path);
	WRL::ComPtr<ID3D11Texture2D> LoadD3DTextureFromFile(const wchar_t* path);
};
